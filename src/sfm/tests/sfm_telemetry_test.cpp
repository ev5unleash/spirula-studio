...
static void test_camm() {
    ...
    check(tm.gyro.size() == 3 && near(tm.gyro[2].x, 0.2, 1e-6) && near(tm.gyro[2].t, 0.4, 1e-9), "camm: gyro value and time");
    check(tm.accel.size() == 3 && near(tm.accel[0].z, 9.81, 1e-5) && near(tm.accel[0].t, 0.1, 1e-9), "camm: accel");
    check(tm.gps.size() == 1 && tm.gps[0].fix && near(tm.gps[0].lon, -79.4, 1e-9) && near(tm.gps[0].speed, 5.0, 1e-5) &&
          near(tm.gps[0].alt, 100.0, 1e-4) && near(tm.gps[0].t, 0.6, 1e-9), "camm: gps");
    check(near(tm.video_duration, 0.7, 1e-9), "camm: movie duration");
}
...
static void test_gpmf() {
    ...
    if (tm.gps.size() == 4) {
        check(near(tm.gps[0].lat, 43.5, 1e-9) && near(tm.gps[0].lon, -79.4, 1e-9) && near(tm.gps[0].alt, 100.0, 1e-9) &&
              near(tm.gps[0].speed, 2.5, 1e-9), "gpmf: GPS5 scaling");
        check(tm.gps[0].fix && near(tm.gps[0].dop, 1.72, 1e-9), "gpmf: GPSF and GPSP");
        check(near(tm.gps[0].unix_time, 1571230484.0, 1e-3), "gpmf: GPSU to unix");
        check(near(tm.gps[0].t, 0.0, 1e-9) && near(tm.gps[2].t, 1.0, 1e-9), "gpmf: GPS anchored to its own first payload");
    }
    check(tm.gravity.size() == 4 && near(tm.gravity[0].y, -1.0, 1e-4) && tm.orientation_axes == "XYZ", "gpmf: GRAV swapped into the ORIN frame");
}
...
static void test_insta360(bool raw) {
    ...
    check(near(tm.frame_readout, 0.0085, 1e-9), tag + "readout");
    check(tm.gyro.size() == 3 && tm.accel.size() == 3, tag + "imu counts");
    if (tm.gyro.size() == 3) {
        check(near(tm.accel[0].z, 9.80665, raw ? 0.01 : 1e-9) && near(tm.accel[0].x, 0, 1e-9), tag + "accel in m/s^2");
        check(near(tm.gyro[0].x, raw ? 1638.0 / 32768 * 2000 * 3.14159265358979 / 180 : 0.1, 1e-6), tag + "gyro rad/s");
        check(near(tm.gyro[0].t, 0.0, 1e-9) && near(tm.gyro[2].t, 0.002, 1e-9), tag + "time from the first frame");
    }
    check(tm.gps.size() == 2, tag + "gps count");
    if (tm.gps.size() == 2) {
        check(tm.gps[0].fix && !tm.gps[1].fix, tag + "fix flag");
        check(near(tm.gps[0].lon, -79.4, 1e-9) && near(tm.gps[0].alt, 64.8, 1e-9) && near(tm.gps[0].speed, 1.5, 1e-9), tag + "gps fields");
        check(near(tm.gps[0].t, 0.5, 1e-6) && near(tm.gps[1].t, 1.5, 1e-6), tag + "gps time from the movie header");
    }
    check(near(tm.video_duration, 44.0, 1e-9), tag + "duration from the movie header");
}
...
static void test_dji() {
    ...
    check(near(tm.frame_readout, 0.024020639, 1e-12), "dji: readout");
    check(tm.accel.size() == 2 && near(tm.accel[0].z, -9.80665, 1e-5) && near(tm.accel[1].t, 0.033366, 1e-6), "dji: accel per frame");
    check(tm.orientation.size() == 6, "dji: 3 quaternions per frame");
    if (tm.orientation.size() == 6) {
        check(near(tm.orientation[0].t, -1.0 / 30 / 3, 1e-9) && near(tm.orientation[1].t, 0.0, 1e-9), "dji: attitude times use the offset");
        check(near(tm.orientation[3].t, 0.033366 - 1.0 / 30 / 3, 1e-6), "dji: second frame");
    }
...
static void test_checks() {
    ...
    check(near(c.accel.rate_hz, 100, 0.5) && near(c.accel_norm_median, 9.80665, 0.05), "check: rate and gravity norm");
    ...
    check(near(c.gps_frozen_fraction, 1.0, 1e-9) && c.gps_distinct == 1 && c.gps_spread_m < 0.01, "check: frozen GPS measured");
