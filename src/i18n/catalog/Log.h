...
SS_MSG(driver_memory_refusal,
    EN("GPU memory needed: {0}; available after safety margin: {1}. Lower the resolution or free GPU memory."),
    JA("必要なGPUメモリ: {0}、安全マージン適用後の空き容量: {1}。解像度を下げるか、GPUメモリを解放してください。"),
    ZH_HANS("所需 GPU 显存：{0}；扣除安全余量后的可用显存：{1}。请降低分辨率或释放 GPU 显存。"),
    ZH_HANT("所需 GPU 記憶體：{0}；扣除安全餘量後的可用記憶體：{1}。請降低解析度或釋放 GPU 記憶體。"),
    KO("필요한 GPU 메모리: {0}; 안전 여유분을 제외한 사용 가능 메모리: {1}. 해상도를 낮추거나 GPU 메모리를 확보하세요."),
    DE("Benötigter GPU-Speicher: {0}; nach Sicherheitsreserve verfügbar: {1}. Verringern Sie die Auflösung oder geben Sie GPU-Speicher frei."),
    FR("Mémoire GPU requise : {0} ; disponible après marge de sécurité : {1}. Réduisez la résolution ou libérez de la mémoire GPU."),
    ES("Memoria de GPU necesaria: {0}; disponible tras el margen de seguridad: {1}. Reduce la resolución o libera memoria de GPU."),
    PT("Memória de GPU necessária: {0}; disponível após a margem de segurança: {1}. Reduza a resolução ou libere memória de GPU."),
    IT("Memoria GPU necessaria: {0}; disponibile dopo il margine di sicurezza: {1}. Riduci la risoluzione o libera memoria GPU."),
    NL("Benodigd GPU-geheugen: {0}; beschikbaar na veiligheidsmarge: {1}. Verlaag de resolutie of maak GPU-geheugen vrij."),
    RU("Требуется памяти GPU: {0}; доступно с учетом резерва безопасности: {1}. Уменьшите разрешение или освободите память GPU."),
    TR("Gerekli GPU belleği: {0}; güvenlik payından sonra kullanılabilir: {1}. Çözünürlüğü düşürün veya GPU belleğini boşaltın."));
SS_MSG(app_memory_refusal,
    EN("GPU memory needed: {0}; available under --memory-limit-gib: {1}. Raise the limit or lower the resolution."),
    JA("必要なGPUメモリ: {0}、--memory-limit-gib で使用可能: {1}。上限を引き上げるか、解像度を下げてください。"),
    ZH_HANS("所需 GPU 显存：{0}；--memory-limit-gib 下的可用显存：{1}。请提高上限或降低分辨率。"),
    ZH_HANT("所需 GPU 記憶體：{0}；--memory-limit-gib 限制下的可用記憶體：{1}。請提高上限或降低解析度。"),
    KO("필요한 GPU 메모리: {0}; --memory-limit-gib 제한 내 사용 가능 메모리: {1}. 제한을 높이거나 해상도를 낮추세요."),
    DE("Benötigter GPU-Speicher: {0}; unter --memory-limit-gib verfügbar: {1}. Erhöhen Sie das Limit oder verringern Sie die Auflösung."),
    FR("Mémoire GPU requise : {0} ; disponible avec --memory-limit-gib : {1}. Augmentez la limite ou réduisez la résolution."),
    ES("Memoria de GPU necesaria: {0}; disponible con --memory-limit-gib: {1}. Aumenta el límite o reduce la resolución."),
    PT("Memória de GPU necessária: {0}; disponível com --memory-limit-gib: {1}. Aumente o limite ou reduza a resolução."),
    IT("Memoria GPU necessaria: {0}; disponibile con --memory-limit-gib: {1}. Aumenta il limite o riduci la risoluzione."),
    NL("Benodigd GPU-geheugen: {0}; beschikbaar onder --memory-limit-gib: {1}. Verhoog de limiet of verlaag de resolutie."),
    RU("Требуется памяти GPU: {0}; доступно при --memory-limit-gib: {1}. Увеличьте лимит или уменьшите разрешение."),
    TR("Gerekli GPU belleği: {0}; --memory-limit-gib kapsamında kullanılabilir: {1}. Limiti artırın veya çözünürlüğü düşürün."));
SS_MSG(memory_telemetry_unavailable,
    EN("GPU memory usage is unavailable, so training was refused instead of guessing a safe limit."),
    JA("GPUメモリ使用量を取得できないため、安全な上限を推測せず、トレーニングを拒否しました。"),
    ZH_HANS("无法获取 GPU 显存使用量，因此已拒绝训练，而不是猜测安全上限。"),
    ZH_HANT("無法取得 GPU 記憶體使用量，因此已拒絕訓練，而非猜測安全上限。"),
    KO("GPU 메모리 사용량을 확인할 수 없어 안전한 한도를 추측하는 대신 학습을 거부했습니다."),
    DE("Die GPU-Speichernutzung ist nicht verfügbar. Das Training wurde daher abgelehnt, statt ein sicheres Limit zu schätzen."),
    FR("L’utilisation de la mémoire GPU est indisponible ; l’entraînement a donc été refusé plutôt que d’estimer une limite sûre."),
    ES("El uso de memoria de la GPU no está disponible, así que se rechazó el entrenamiento en lugar de estimar un límite seguro."),
    PT("O uso da memória da GPU não está disponível; por isso, o treinamento foi recusado em vez de estimar um limite seguro."),
    IT("L'utilizzo della memoria GPU non è disponibile, quindi l'addestramento è stato rifiutato anziché stimare un limite sicuro."),
    NL("Het GPU-geheugengebruik is niet beschikbaar. Daarom is de training geweigerd in plaats van een veilige limiet te schatten."),
    RU("Данные об использовании памяти GPU недоступны, поэтому обучение было отклонено вместо попытки угадать безопасный лимит."),
    TR("GPU bellek kullanımı bilgisi mevcut olmadığından güvenli bir limit tahmin etmek yerine eğitim reddedildi."));
SS_MSG(memory_telemetry_error,
    EN("The GPU memory query failed, so training was refused before allocation."),
    JA("GPUメモリの照会に失敗したため、割り当て前にトレーニングを拒否しました。"),
    ZH_HANS("GPU 显存查询失败，因此在分配显存前已拒绝训练。"),
    ZH_HANT("GPU 記憶體查詢失敗，因此在配置記憶體前已拒絕訓練。"),
    KO("GPU 메모리 조회에 실패하여 할당 전에 학습을 거부했습니다."),
    DE("Die GPU-Speicherabfrage ist fehlgeschlagen. Das Training wurde daher vor der Zuweisung abgelehnt."),
    FR("La requête de mémoire GPU a échoué ; l’entraînement a donc été refusé avant l’allocation."),
    ES("La consulta de memoria de la GPU falló, así que se rechazó el entrenamiento antes de la asignación."),
    PT("A consulta de memória da GPU falhou; por isso, o treinamento foi recusado antes da alocação."),
    IT("La richiesta relativa alla memoria GPU non è riuscita, quindi l'addestramento è stato rifiutato prima dell'allocazione."),
    NL("De GPU-geheugenquery is mislukt. Daarom is de training vóór de toewijzing geweigerd."),
    RU("Запрос памяти GPU завершился с ошибкой, поэтому обучение было отклонено до выделения памяти."),
    TR("GPU bellek sorgusu başarısız olduğundan eğitim bellek ayrılmadan önce reddedildi."));
SS_MSG(bad_memory_limit,
    EN("memory_limit_gib must be greater than zero."),
    JA("memory_limit_gib は0より大きい値にする必要があります。"),
    ZH_HANS("memory_limit_gib 必须大于零。"),
    ZH_HANT("memory_limit_gib 必須大於零。"),
    KO("memory_limit_gib는 0보다 커야 합니다."),
    DE("memory_limit_gib muss größer als null sein."),
    FR("memory_limit_gib doit être supérieur à zéro."),
    ES("memory_limit_gib debe ser mayor que cero."),
    PT("memory_limit_gib deve ser maior que zero."),
    IT("memory_limit_gib deve essere maggiore di zero."),
    NL("memory_limit_gib moet groter zijn dan nul."),
    RU("memory_limit_gib должен быть больше нуля."),
    TR("memory_limit_gib sıfırdan büyük olmalıdır."));
