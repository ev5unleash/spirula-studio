...
SS_MSG(queued_job_gpu_help,
    EN("GPU for new queued dataset jobs. The choice is saved when you submit a job."),
    JA("新しく待機するデータセットジョブが使う GPU です。選択はジョブの送信時に保存されます。"),
    ZH_HANS("用于新排队的数据集任务。提交任务时会保存该选择。"),
    ZH_HANT("用於新排隊的資料集工作。提交工作時會儲存該選擇。"),
    KO("새로 대기하는 데이터셋 작업에 사용할 GPU입니다. 작업을 제출할 때 선택이 저장됩니다."),
    DE("GPU für neue eingereihte Dataset-Aufträge. Die Auswahl wird beim Absenden eines Auftrags gespeichert."),
    FR("GPU pour les nouvelles tâches de jeu de données en file d'attente. Le choix est enregistré lorsque vous soumettez une tâche."),
    ES("GPU para las nuevas tareas de conjunto de datos en cola. La selección se guarda al enviar una tarea."),
    PT("GPU para novas tarefas de conjunto de dados na fila. A escolha é guardada ao enviar uma tarefa."),
    IT("GPU per le nuove attività del set di dati in coda. La scelta viene salvata quando invii un'attività."),
    NL("GPU voor nieuwe dataset-taken in de wachtrij. De keuze wordt opgeslagen wanneer je een taak indient."),
    RU("GPU для новых заданий с наборами данных в очереди. Выбор сохраняется при отправке задания."),
    TR("Yeni kuyruğa alınan veri kümesi işleri için GPU. Seçim, işi gönderdiğinizde kaydedilir."));
