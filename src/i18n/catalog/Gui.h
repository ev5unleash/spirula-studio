...
SS_MSG(resume_run,
    ...
    ZH_HANT("從執行資料夾繼續…"),
    KO("실행 폴더에서 재개…"),
    DE("Aus dem Laufordner fortsetzen …"),
    FR("Reprendre depuis un dossier d’exécution…"),
    ES("Reanudar desde una carpeta de ejecución…"),
    PT("Retomar de uma pasta de execução…"),
    IT("Riprendi da una cartella di esecuzione…"),
    NL("Hervatten vanuit een runmap…"),
...
SS_MSG(resume_training,
    ...
    ES("Reanudar el entrenamiento"),
    PT("Retomar o treinamento"),
    IT("Riprendi l’addestramento"),
    NL("Training hervatten"),
    RU("Продолжить обучение"),
    TR("Eğitimi sürdür"));

SS_MSG(resume_checkpoint,
    EN("Checkpoint: {0} (step {1})"),
    JA("チェックポイント: {0}（ステップ {1}）"),
    ZH_HANS("检查点：{0}（第 {1} 步）"),
    ZH_HANT("檢查點：{0}（第 {1} 步）"),
    KO("체크포인트: {0} ({1}단계)"),
    DE("Checkpoint: {0} (Schritt {1})"),
    FR("Point de contrôle : {0} (étape {1})"),
    ES("Punto de control: {0} (paso {1})"),
    PT("Ponto de verificação: {0} (etapa {1})"),
    IT("Checkpoint: {0} (passo {1})"),
    NL("Checkpoint: {0} (stap {1})"),
    RU("Контрольная точка: {0} (шаг {1})"),
    TR("Denetim noktası: {0} (adım {1})"));

SS_MSG(resume_clear,
    EN("Start fresh instead"),
    JA("新しく始める"),
    ZH_HANS("改为重新开始"),
    ZH_HANT("改為重新開始"),
    KO("새로 시작"),
    DE("Stattdessen neu beginnen"),
    ...
    NL("Opnieuw beginnen"),
    RU("Начать заново"),
    TR("Bunun yerine baştan başla"));

SS_MSG(resume_extend_steps,
    EN("Increase total iterations above {0} to continue training."),
    JA("学習を続けるには、総反復回数を {0} より大きくしてください。"),
    ZH_HANS("要继续训练，请将总迭代次数增加到 {0} 以上。"),
    ZH_HANT("要繼續訓練，請將總迭代次數增加到 {0} 以上。"),
    KO("학습을 계속하려면 총 반복 횟수를 {0}보다 크게 늘리세요."),
    DE("Erhöhen Sie die Gesamtzahl der Iterationen über {0}, um fortzufahren."),
    FR("Augmentez le nombre total d’itérations au-delà de {0} pour continuer."),
    ES("Aumenta las iteraciones totales por encima de {0} para continuar."),
    PT("Aumente o total de iterações acima de {0} para continuar."),
    IT("Aumenta le iterazioni totali oltre {0} per continuare."),
    NL("Verhoog het totale aantal iteraties boven {0} om door te gaan."),
    RU("Увеличьте общее число итераций выше {0}, чтобы продолжить обучение."),
    TR("Eğitime devam etmek için toplam yinelemeyi {0} değerinin üzerine çıkarın."));

SS_MSG(resume_failed,
    EN("Cannot resume training from {0}: {1}"),
    JA("{0} から学習を再開できません: {1}"),
    ZH_HANS("无法从 {0} 恢复训练：{1}"),
    ZH_HANT("無法從 {0} 繼續訓練：{1}"),
    KO("{0}에서 학습을 재개할 수 없습니다: {1}"),
    DE("Training kann nicht aus {0} fortgesetzt werden: {1}"),
    FR("Impossible de reprendre l’entraînement depuis {0} : {1}"),
    ES("No se puede reanudar el entrenamiento desde {0}: {1}"),
    PT("Não é possível retomar o treinamento de {0}: {1}"),
    IT("Impossibile riprendere l’addestramento da {0}: {1}"),
    NL("Kan de training niet hervatten vanuit {0}: {1}"),
    RU("Не удалось продолжить обучение из {0}: {1}"),
    TR("{0} konumundan eğitime devam edilemiyor: {1}"));

SS_MSG(resume_busy,
    EN("Finish the current operation before opening a training run."),
    JA("学習実行フォルダを開く前に、現在の操作を完了してください。"),
    ZH_HANS("打开训练运行前，请先完成当前操作。"),
    ZH_HANT("開啟訓練執行前，請先完成目前的操作。"),
    KO("학습 실행 폴더를 열기 전에 현재 작업을 완료하세요."),
    DE("Beenden Sie den aktuellen Vorgang, bevor Sie einen Trainingslauf öffnen."),
    FR("Terminez l’opération en cours avant d’ouvrir un entraînement."),
    ES("Termina la operación actual antes de abrir un entrenamiento."),
    PT("Conclua a operação atual antes de abrir uma execução de treinamento."),
    IT("Completa l’operazione corrente prima di aprire un’esecuzione di addestramento."),
    NL("Rond de huidige bewerking af voordat u een trainingsrun opent."),
    RU("Завершите текущую операцию перед открытием запуска обучения."),
    TR("Bir eğitim çalışmasını açmadan önce mevcut işlemi tamamlayın."));

SS_MSG(resume_dataset_missing,
    EN("The saved dataset path does not exist: {0}"),
    JA("保存されたデータセットのパスが存在しません: {0}"),
    ZH_HANS("保存的数据集路径不存在：{0}"),
    ZH_HANT("儲存的資料集路徑不存在：{0}"),
    KO("저장된 데이터셋 경로가 존재하지 않습니다: {0}"),
    DE("Der gespeicherte Datensatzpfad existiert nicht: {0}"),
    FR("Le chemin du jeu de données enregistré n’existe pas : {0}"),
    ES("La ruta del conjunto de datos guardado no existe: {0}"),
    PT("O caminho do conjunto de dados salvo não existe: {0}"),
    IT("Il percorso del set di dati salvato non esiste: {0}"),
    NL("Het opgeslagen datasetpad bestaat niet: {0}"),
    RU("Сохранённый путь к набору данных не существует: {0}"),
    TR("Kayıtlı veri kümesi yolu mevcut değil: {0}"));

SS_MSG(resume_dismiss,
    EN("Dismiss"),
    JA("閉じる"),
    ZH_HANS("关闭"),
    ZH_HANT("關閉"),
...
SS_MSG(recovery_title,
    EN("Recover interrupted job?"),
    JA("中断したジョブを再開しますか？"),
    ZH_HANS("恢复中断的任务吗？"),
    ZH_HANT("恢復中斷的工作嗎？"),
    KO("중단된 작업을 재개할까요?"),
    DE("Unterbrochenen Auftrag fortsetzen?"),
    FR("Reprendre la tâche interrompue ?"),
    ES("¿Reanudar el trabajo interrumpido?"),
    PT("Retomar a tarefa interrompida?"),
    IT("Riprendere il lavoro interrotto?"),
    NL("Onderbroken taak hervatten?"),
    RU("Продолжить прерванное задание?"),
    TR("Kesintiye uğrayan iş sürdürülsün mü?"));

SS_MSG(recovery_body,
    EN("The last GUI job did not finish. Resume it now?"),
    JA("前回の GUI ジョブは完了しませんでした。今すぐ再開しますか？"),
    ZH_HANS("上一次 GUI 任务未完成。现在继续吗？"),
    ZH_HANT("上一次 GUI 工作未完成。現在繼續嗎？"),
    KO("마지막 GUI 작업이 완료되지 않았습니다. 지금 재개할까요?"),
    DE("Der letzte GUI-Auftrag wurde nicht abgeschlossen. Jetzt fortsetzen?"),
    FR("La dernière tâche GUI n’est pas terminée. La reprendre maintenant ?"),
    ES("El último trabajo de la GUI no terminó. ¿Reanudarlo ahora?"),
    PT("A última tarefa da GUI não terminou. Retomá-la agora?"),
    IT("L’ultimo lavoro GUI non è terminato. Riprenderlo ora?"),
    NL("De laatste GUI-taak is niet voltooid. Nu hervatten?"),
    RU("Последнее задание GUI не завершилось. Продолжить сейчас?"),
    TR("Son GUI işi tamamlanmadı. Şimdi sürdürülsün mü?"));
