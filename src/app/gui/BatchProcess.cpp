...
void check_dataset_stage(const BatchRow& row, const BatchCapabilities& caps,
    ...
    if (s.colmap_engine || s.sfm.prep.force_external_masking)
        out.push_back(issue_of(msg::scheduler_external_interactive, kSt, true));
...
void check_train_stage(const BatchRow& row, const BatchCapabilities& caps,
    ...
    if (made_here && batch_num_runs(row) > 1)
        out.push_back(issue_of(msg::chk_linked_train_count, kSt, true));
...
void check_mesh_stage(const BatchRow&, std::vector<BatchIssue>& out) {
    out.push_back(issue_of(msg::chk_mesh_interactive, BatchStage::Mesh, true));
}
    ...
    try {
        ...
        if (const JsonValue* jobs = root.find("jobs"); jobs && jobs->is_array()) {
            for (const JsonValue& j : jobs->arr) {
                ...
                if (const JsonValue* v = j.find("device"))
