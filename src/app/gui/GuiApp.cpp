...
void GuiApp::load_preset_file(const std::string& path) {
    ...
    // down with it. Refuse while native work or a batch is running.
...
void GuiApp::add_batch_row(const std::string& dataset) {
    ...
    BatchRow r;
    r.dataset = dataset;
    r.device = _scheduled_device_request;
...
void GuiApp::start_batch(bool skip_invalid) {
    ...
    auto absolute_config_paths = [&](TrainConfig& cfg) {
        auto absolute = [&](std::string& path) {
            if (path.empty()) return;
            const fs::path value = fs::absolute(fs::u8path(path), ec);
            if (!ec) path = value.u8string();
            ec.clear();
        };
        absolute(cfg.data);
    ...
    auto set_phase_device = [](app::sched::Phase& phase,
                               const std::string& device,
                               const std::string& name) {
        phase.planned_device = device;
        phase.planned_device_name = name;
    };
    auto geometry_phase = [&](const GeometryJob& geometry,
        ...
        phase.optional = true;
        phase.output = (fs::path(workspace) /
                        (geometry.want_normal ? "normals" : "depths")).u8string();
        set_phase_device(phase, device, device_name);
        phase.args.push_back(workspace);
        const std::string image_arg =
            image_dir == (fs::path(workspace) / "images").u8string()
                ? "images" : image_dir;
        phase.args.insert(phase.args.end(), {"--image-dir", image_arg,
                                             "--model", geometry.model,
                                             "--max-size", std::to_string(geometry.max_size),
                                             "--num-tokens", std::to_string(geometry.num_tokens)});
        if (geometry.want_depth) phase.args.push_back("--depth");
        if (!geometry.want_normal) phase.args.push_back("--no-normal");
        phase.args.insert(phase.args.end(),
                          {"--normal-format", geometry.normal_jpg ? "jpg" : "png",
                           "--jpeg-quality", std::to_string(geometry.jpeg_quality),
                           "--depth-units", geometry.depth_mm ? "mm" : "relative"});
        const char* ray[] = {"auto", "yes", "no"};
        if (geometry.ray_depth >= 0 && geometry.ray_depth <= 2)
            phase.args.insert(phase.args.end(), {"--ray-depth", ray[geometry.ray_depth]});
        if (geometry.split >= 0 && geometry.split <= 2)
            phase.args.insert(phase.args.end(), {"--split", ray[geometry.split]});
        if (geometry.overwrite) phase.args.push_back("--overwrite");
        if (!geometry.image_gamut.empty())
            phase.args.insert(phase.args.end(), {"--image-gamut", geometry.image_gamut});
        if (geometry.image_is_linear.has_value())
            phase.args.push_back(*geometry.image_is_linear ? "--image-linear"
                                                           : "--no-image-linear");
        return phase;
    };
    ...
    for (BatchRow& row : _batch) {
        if (!row.enabled || batch_has_error(row.issues)) continue;

        const std::string request = row.device.empty() ? _scheduled_device_request
                                                        : row.device;
        std::string device, device_name, error;
        if (!resolve_scheduled_device(
                request, !row.device.empty() || _scheduled_device_choice_set,
                device, device_name, error)) {
            log(error);
            continue;
        }

        if (row.does(BatchStage::Dataset)) {
            std::vector<PrepInput> sources;
            DatasetSettings settings;
            std::string workspace;
            if (!batch_build_dataset_job(row, _ffmpeg_exe, sources, settings,
                                         workspace, error)) {
                log(error);
                continue;
            }

            app::PrepJob frozen = settings.sfm.prep;
            frozen.inputs = std::move(sources);
            frozen.workspace = workspace;
            frozen.ffmpeg_exe = _ffmpeg_exe;
            frozen.python_exe = _python_exe;
            frozen.device = device;
            frozen.mask_prompt = settings.mask.prompt;
            frozen.mask_negative_prompt = settings.mask.negative_prompt;
            frozen.mask_keep_subject = settings.mask.keep_subject;
            frozen.mask_dilate_ratio = settings.mask.boundary_ratio();
            frozen.mask_max_image_size = settings.mask.max_image_size;
            frozen.mask_threshold = settings.mask.threshold;
            frozen.mask_nms = settings.mask.nms;
            frozen.mask_clicks.clear();
            frozen.flip_found_masks = settings.use_found_masks &&
                                      frozen.flip_found_masks;
            if (!settings.border_enable)
                for (PrepInput& input : frozen.inputs)
                    input.stencil = app::FrameStencil{};
            if (const ModelEntry* model = find_model(settings.mask_model_id)) {
                frozen.mask_model_path = model_path(*model);
                frozen.mask_model_name = model->legacy_name;
            }
            if (!app::worker::freeze_prep_job(frozen, work_dir, error) ||
                app::worker::reject_external_masking(frozen, error)) {
                log(error.empty() ? msg::scheduler_external_interactive.get()
                                  : error);
                continue;
            }

            SfmJob scheduled_sfm = settings.sfm;
            ...
            set_phase_device(prep, device, device_name);
            ...
            if (scheduled_sfm.geometry.enable)
                submit.phases.push_back(geometry_phase(
                    scheduled_sfm.geometry, frozen.workspace, planned.image_dir,
                    device, device_name));

            if (row.does(BatchStage::Train)) {
                TrainConfig cfg;
                std::string preset_base;
                if (!batch_build_train_config(row, 0, frozen.workspace,
                                              planned.image_dir, planned.mask_dir,
                                              planned.mask_dir_flipped, cfg,
                                              preset_base, error)) {
                    log(error);
                    continue;
                }
                ...
                submit.phases.push_back(std::move(train));
            }
        ...
        if (!row.does(BatchStage::Train)) continue;
        for (int variant = 0; variant < batch_num_runs(row); ++variant) {
            TrainConfig cfg;
            std::string preset_base;
            if (!batch_build_train_config(row, variant, row.dataset, {}, {},
                                          false, cfg, preset_base, error)) {
                log(error);
                continue;
            }
            absolute_config_paths(cfg);
            app::sched::SubmitOpts submit;
            submit.phase = "train";
        ...
        for (const std::string& id : row.scheduler_ids) {
            ...
            if (job->state == app::sched::JobState::Queued) {
                _scheduler.cancel(id);
                       ...
                       job->state == app::sched::JobState::Running ||
                       job->state == app::sched::JobState::Stopping) {
                if (save && job->phase == "train") _scheduler.stop_and_save(id);
                else _scheduler.force_stop(id);
            }
        }
    }
}
...
void GuiApp::pump_source_probes() {
    ...
    bool all_pano = true;
    for (const PrepInput& s : _sources)
        all_pano = all_pano && s.pano360.valid();
...
void GuiApp::draw_job_device_picker(std::string& request, const char* id,
    ...
    for (size_t i = 0; i < _native_devices.size(); ++i)
        if (!request.empty() && _native_devices[i].uuid == request) {
            shown = _native_devices[i].name + " [" + std::to_string(i) + "]";
            break;
        }
...
void GuiApp::draw_batch_device_picker(BatchRow& row) {
    draw_job_device_picker(row.device, "##batch_device");
}
...
void GuiApp::draw_scheduler_queue() {
    ...
    for (const app::sched::Job& job : _scheduler_jobs) {
        ...
        if (job.state == app::sched::JobState::Queued) {
            std::string device = job.device;
            bool changed = false;
            ui::Text(dmsg::queued_job_gpu);
            ImGui::SameLine();
            draw_job_device_picker(device, "##queued_device", &changed);
            ui::help_on_hover(dmsg::queued_job_gpu_help);
            if (changed) {
                std::string resolved;
                std::string name;
                std::string error;
                if (resolve_scheduled_device(device, !device.empty(), resolved,
                                             name, error))
                    _scheduler.set_device(job.job_id, resolved, name);
            }
...
BatchStatus row_status(const BatchRow& row,
                       const std::vector<app::sched::Job>& jobs,
                       std::string& detail) {
    if (row.scheduler_ids.empty()) return BatchStatus::Pending;
    bool all_done = true, running = false, failed = false, stopped = false;
    for (const std::string& id : row.scheduler_ids) {
        const auto it = std::find_if(jobs.begin(), jobs.end(),
            [&](const app::sched::Job& job) { return job.job_id == id; });
        if (it == jobs.end()) {
            all_done = false;
            continue;
        }
        const app::sched::Job& job = *it;
        running = running || job.state == app::sched::JobState::Starting ||
                  job.state == app::sched::JobState::Running ||
                  job.state == app::sched::JobState::Stopping;
        failed = failed || job.state == app::sched::JobState::Failed ||
                 job.state == app::sched::JobState::Interrupted ||
                 job.state == app::sched::JobState::Blocked;
        stopped = stopped || job.state == app::sched::JobState::Stopped;
        all_done = all_done && job.state == app::sched::JobState::Succeeded;
        if (!job.error.empty()) detail = job.error;
        else if (!job.output_dir.empty()) detail = job.output_dir;
    }
...
void GuiApp::draw_batch() {
    ...
    ImGui::BeginDisabled(_batch.empty() || _batch_active);
...
void GuiApp::draw_batch_rows() {
    ...
    if (remove >= 0) {
        for (const std::string& id : _batch[remove].scheduler_ids)
...
void GuiApp::draw_batch_row(BatchRow& row, int index, int& remove, int& move) {
    ...
    // ---- status, and why ----
    std::string detail;
    const BatchStatus st = row_status(row, _scheduler_jobs, detail);
...
void GuiApp::draw_batch_row_common_dataset(BatchRow& row, int index) {
    ...
    ui::Text(msg::menu_device);
    ImGui::SetNextItemWidth(px(260.0f));
    const std::string old_device = row.device;
    draw_batch_device_picker(row);
    if (row.device != old_device) batch_edited();
}
...
void GuiApp::draw_batch_plan() {
    ...
    const std::vector<BatchTask> plan = batch_plan(_batch);
    if (plan.empty()) {
