...
SourceProbeInfo probe(const std::string& path, const std::string& ffmpeg_exe,
    ...
    if (!cancel.load() && is_pano360_path(path)) {
        const app::Pano360Probe pano =
            app::probe_pano360(ffmpeg_exe, path, cancel);
        out.pano360 = pano.layout;
        out.pano360_unsupported = pano.unsupported;
    }
    out.done = true;
    return out;
}
