...
// Editable batch rows and their scheduler submission inputs.
//
// The list is data, not a runner. JobScheduler owns execution and recovery;
// this module owns persistence, preflight, presets, and plan summaries.
...
// One planned stage shown by the batch preview. Execution state lives only in
// JobScheduler.
struct BatchTask {
    int row = 0;
...
// An empty run list means one run on the built-in default.
...
// Serialize every resolved training option so the worker cannot reapply a
// preset macro or inherit GUI state.
