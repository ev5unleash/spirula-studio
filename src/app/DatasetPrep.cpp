...
int run_process(const std::vector<std::string>& argv,
    ...
    proc::ProcessOptions options;
    options.argv = argv;
    options.cwd = cwd;
    options.on_line = on_line;
    options.cancel = &cancel;
    const proc::ProcessResult result = proc::run_process(options);
    if (result.outcome == proc::ProcessOutcome::Cancelled) return kCancelled;
    if (result.outcome == proc::ProcessOutcome::SpawnFailed) return kSpawnFailed;
    return result.exit_code;
}

bool command_exists(const std::string& exe) {
    return proc::command_exists(exe);
}
...
// Follow directory symlinks: prepared captures commonly link images/ and masks/
// into raw storage, and the default iterator would otherwise miss them.
...
// Lexical relative path avoids symlink resolution sending generated masks
...
// Derive masker frame ids from extractor filename suffixes. Reset numbering
// across camera folders so prompt seeds remain aligned.
...
public:
    ...
    RateLimitedProgress(const DatasetPrepSinks& sinks, Stage stage,
                        const spirula::i18n::Msg& noun, StageTally& tally)
        : _sinks(&sinks), _stage(stage), _noun(&noun), _tally(&tally),
    ...
    void update(int64_t in_segment, bool force = false) {
        ...
        if (_sinks->count) _sinks->count(_stage, done, _total);
        const double since =
        ...
        // Keep rate and estimate in one localized sentence; their order differs by
        // language.
...
private:
    ...
    void say(const std::string& s) {
        if (_sinks->detail) _sinks->detail(_stage, s);
        if (_sinks->log) _sinks->log(_stage, s, /*detail=*/true);
    }
...
bool ffmpeg_probe_video(const std::string& ffmpeg_exe, const std::string& path,
    ...
    // Parse ffmpeg's stream table from stderr; ffprobe is not required because the
    // configured tool path is only ffmpeg.
...
void resolve_photo_folder(const std::string& picked, std::string& images,
    ...
    images = p.string();
    masks.clear();
    // For a dataset folder, index images/ rather than the folder containing masks
...
void DatasetPrep::log(const std::string& s, bool detail) {
    if (_sinks.log) _sinks.log(_stage, s, detail);
}
...
bool DatasetPrep::run(const PrepJob& job_in, PrepResult& out, std::string& error,
    ...
    if (job.mask_enable) {
        for (size_t i = 0; i < job.inputs.size(); i++) {
            // Redo only generated masks; supplied masks may live outside the workspace and
...
bool DatasetPrep::gather_photos(const PrepJob& job, const PrepInput& in,
    ...
    for (const Tree& t : trees) {
        ...
        RateLimitedProgress progress(_sinks, Stage::Frames, *t.counted,
                                     _frames_tally);
        std::vector<std::thread> pool;
...
bool DatasetPrep::generate_masks_builtin(const PrepJob& job, const PrepInput& in,
    ...
    // Use memory tracking only for requested video runs or prompted objects; photo
    // inputs otherwise must not share state across unrelated frames.
    ...
    RateLimitedProgress progress(_sinks, Stage::Masks, lmsg::noun_images_masked,
                                 _masks_tally);
    for (size_t k = 0; k < todo.size(); k++) {
        ...
        if (_sinks.frame) {
            ...
            f.width = img.width;
            f.height = img.height;
            f.mask = mask.data.data();
            // The callback runs before the image and mask buffers move on.
            _sinks.frame(f);
        }
        app::WriteJob wj;
        ...
        // Increment before reporting so the final mask is counted once, with defined
        // evaluation order.
        ++done;
...
// Detect missing Python packages from the fallback's output, not its zero exit
// code, which is also used for the install hint.
