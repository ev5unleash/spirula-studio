...
| `gui/BatchProcess.h/.cpp` | **Scheduled batch inputs**: editable, persisted rows for native dataset creation and training. `batch_check_row()` rejects unavailable inputs/models, colliding workspaces, external COLMAP/Python masking, scheduled meshing, and more than one training run linked to a newly built dataset. Existing datasets may have several independent training runs. `batch_build_dataset_job()` and `batch_build_train_config()` freeze row presets and overrides; `JobScheduler` alone executes the resulting worker phases on the row's selected device. |
...
| `gui/Subprocess.h/.cpp` | GUI adapter over `app/Subprocess.h/.cpp`, the canonical cross-platform process lifecycle. The GUI layer keeps only typed-command helpers: `split_args()`, `safe_arg()` and `command_argv()`. |
...
logs; its rows do not attach to the foreground viewport. Scheduler state owns
path leases, phase recovery, and the prep → SfM → geometry → train → publish
sequence.
